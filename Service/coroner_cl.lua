...
AddEventHandler('pd5m:service:initcoroner', function()
	...
	AddTextComponentSubstringPlayerName("It will arrive in approximately ~o~2~s~ minutes.")
	...
	local coroner = CreateVehicle(vehiclehash, spawnCoords, spawnHeading, true, false)
	--local coronerdriver = CreatePed(26, drivermodel, stationvec.x, stationvec.y, stationvec.z+2.0, stationvec.angle, true, true)
	local coronerdriver = CreatePedInsideVehicle(coroner, 26, drivermodel, -1, true, true)
	...
	SetBlipSprite(coronerblip, 61)
	SetBlipFlashes(coronerblip, true)
	SetBlockingOfNonTemporaryEvents(coronerdriver, true)
	ShowHeadingIndicatorOnBlip(coronerblip, true)
...
AddEventHandler('pd5m:service:coronerapproach', function(coroner, coronerdriver, coronerblip, station)
	...
	TaskVehicleDriveToCoordLongrange(coronerdriver, coroner, tarx, tary, tarz, 50.0, 831, 2.0)
		...
		if IsVehicleStopped(coroner) and not IsVehicleStoppedAtTrafficLights(coroner) then
			...
			if IsPointOnRoad(corx, cory, corz, coroner) then
				...
				if n > 400 then
					...
					SetVehicleFixed(coroner)
					SetVehicleOnGroundProperly(coroner)
					TaskVehicleDriveToCoordLongrange(coronerdriver, coroner, tarx, tary, tarz, 40.0, 831, 2.0)
				end
			else
				...
				if n > 400 then
					...
					SetVehicleFixed(coroner)
					SetVehicleOnGroundProperly(coroner)
					TaskVehicleDriveToCoordLongrange(coronerdriver, coroner, tarx, tary, tarz, 30.0, 831, 2.0)
				end
			end
		end
		Wait(1000)
...
AddEventHandler('pd5m:service:coroneratscene', function(coroner, coronerdriver, coronerblip, station)
	...
	TaskVehicleDriveToCoord(coronerdriver, coroner, tarx, tary, tarz, 30.0, 0, vehiclehash, 831, 2.0, true)
	...
	if flagcalledcoroner then
		...
		TaskVehicleDriveToCoord(coronerdriver, coroner, tarx, tary, tarz, 20.0, 0, vehiclehash, 831, 2.0, true)
		...
		if flagcalledcoroner then
			...
			if #DeadPedList > 0 then
				...
				for i, target in ipairs(DeadPedList) do
					if DoesEntityExist(target) then
						...
						if DoesEntityExist(target) then
							TriggerServerEvent('pd5m:syncsv:TaskPlayAnim', coronerdriver, 'anim@heists@narcotics@trash', 'pickup', 8.0, 8.0, -1, 0, 0.0, 0, 0, 0)
							Wait(500)
							SetBlockingOfNonTemporaryEvents(target, false)
							SetEntityAsNoLongerNeeded(target)
							local x,y,z = table.unpack(GetEntityCoords(coronerdriver))
							bag = CreateObject(GetHashKey("prop_rub_binbag_sd_01"), x, y, z+0.2,  true,  true, true)
							AttachEntityToEntity(bag, coronerdriver, GetPedBoneIndex(coronerdriver, 0xFA60), 0.2, 0.08, 0.2, -45.0, 290.0, 0.0, true, true, false, true, 1, true)
			...
			TriggerServerEvent('cl-police:server:Pay', "coroner")
