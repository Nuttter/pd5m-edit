...
function GetPedInDirection(coordFrom, coordTo)
	local rayHandle = StartShapeTestRay(coordFrom.x, coordFrom.y, coordFrom.z, coordTo.x, coordTo.y, coordTo.z, 4, GetPlayerPed(-1), 4)
...
function GetVehInDirection(coordFrom, coordTo)
	local rayHandle = StartShapeTestRay(coordFrom.x, coordFrom.y, coordFrom.z, coordTo.x, coordTo.y, coordTo.z, 2, GetPlayerPed(-1), 7)
...
function GetVehicleInFrontOfPlayer()
    ...
    local start = playerCoords + forwardVector * 1.5
    local stop = playerCoords + forwardVector * 15.0

    local rayHandle = StartShapeTestCapsule(start, stop, 2.0, 10, playerPed, 7)
    local _,flag_VehHit,VehCoords,_,VehHit = GetShapeTestResult(rayHandle)
...
function SyncPedAndVeh(target, targetveh)
	...
	if TargetNetID ~= 0 and TargetVehNetID ~= 0 then
		local pedEntry = ClientPedConfigList[TargetFlagListIndex]
		if not pedEntry then
			print("ERROR: ClientPedConfigList[" .. tostring(TargetFlagListIndex) .. "] is nil.")
			return 0, 0 -- or return TargetFlagListIndex, TargetVehFlagListIndex if you want to keep those values
		end
	
		local PedGender = pedEntry.PedGender
		...
		if not ClientPedConfigList[TargetFlagListIndex] then
			print("Error: ClientPedConfigList[" .. tostring(TargetFlagListIndex) .. "] is nil.")
			return
		end
		Wait(100)
